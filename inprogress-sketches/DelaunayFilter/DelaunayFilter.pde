...
void setup() {
  ...
  smallMotionImage = createImage(int(artScreen.motionImage.width / 3f), int(artScreen.motionImage.height / 3f), RGB); 
}

void draw() {
  background(0);
  
  smallMotionImage.copy(artScreen.motionImage, 0, 0, artScreen.motionImage.width, artScreen.motionImage.height, 0, 0, smallMotionImage.width, smallMotionImage.height); 
  ...
  for (int i = 0; i < tSize; i++) {
    ...
    colors[i] = smallMotionImage.get(int(c.x), int(c.y));
  }
