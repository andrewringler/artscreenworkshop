...
class ComputerVision {
  ...
  public ComputerVision(ArtScreen artScreen, PApplet p) {
    this.artScreen = artScreen;
    this.p = p;
    
    previousProcessingFrame = p.createImage(ArtScreen.DEFAULT_CAPTURE_WIDTH / 4, ArtScreen.DEFAULT_CAPTURE_HEIGHT / 4, RGB);
    processingFrame = p.createImage(ArtScreen.DEFAULT_CAPTURE_WIDTH / 4, ArtScreen.DEFAULT_CAPTURE_HEIGHT / 4, RGB);
    processingFrameOpenCV = p.createImage(ArtScreen.DEFAULT_CAPTURE_WIDTH / 4, ArtScreen.DEFAULT_CAPTURE_HEIGHT / 4, RGB);
    ...
    for (int x = 0; x < previousProcessingFrame.width; x++) {
      for (int y = 0; y < previousProcessingFrame.height; y++) {
        ...
        if (change > MIN_PIXEL_CHANGE_THRESHOLD) {
          ...
          artScreen.motionImage.pixels[loc] = color((int)changeB);
          motionPixels.add(new MotionPixel(new PVector(x, y), changeB));
        } else {
          artScreen.motionImage.pixels[loc] = color(0);
        }
      }
    }
